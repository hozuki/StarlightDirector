...
namespace StarlightDirector.App.UI.Forms {
    public partial class FBeatmapStats : Form {
        ...
        private void FBeatmapInfo_Load(object sender, EventArgs e) {
            ...
            listView.Columns.Add(LanguageManager.TryGetString("fbeatmapstats.listview.column.name.text") ?? "Name");
            listView.Columns.Add(LanguageManager.TryGetString("fbeatmapstats.listview.column.value.text") ?? "Value");

            var groupGeneral = listView.Groups.Add("General", LanguageManager.TryGetString("fbeatmapstats.listview.header.general.text") ?? "General");
            var it = listView.Items.Add(LanguageManager.TryGetString("fbeatmapstats.listview.item.project_file_name.text") ?? "Project file name");
            it.SubItems.Add(project.SaveFileName ?? string.Empty);
            it.Group = groupGeneral;
            it = listView.Items.Add(LanguageManager.TryGetString("fbeatmapstats.listview.item.music_file_name.text") ?? "Music file name");
            ...
            foreach (var difficulty in difficulties) {
                ...
                var headerTextFormat = LanguageManager.TryGetString("fbeatmapstats.listview.header.items.text_template") ?? "Difficulty: {0}";
                var text = string.Format(headerTextFormat, difficultyDescription);
                var group = listView.Groups.Add(text, text);
                var it1 = listView.Items.Add(LanguageManager.TryGetString("fbeatmapstats.listview.item.measures.text") ?? "Measures");
                it1.SubItems.Add(numberOfBars.ToString());
                var it2 = listView.Items.Add(LanguageManager.TryGetString("fbeatmapstats.listview.item.notes.text") ?? "Notes");
                it2.SubItems.Add(numberOfNotes.ToString());
                var it3 = listView.Items.Add(LanguageManager.TryGetString("fbeatmapstats.listview.item.duration.text") ?? "Duration");
