...
namespace OpenCGSS.Director.Core.Interop {
    internal static class NativeMethods {
        ...
        [DllImport("user32.dll", CallingConvention = CallingConvention.StdCall, PreserveSig = true, SetLastError = true)]
        internal static extern bool TrackPopupMenu(IntPtr hMenu, uint uFlags, int x, int y, int nReserved, IntPtr hWnd, ref NativeStructures.RECT prcRect);
        ...
        internal static extern int SetWindowTheme(IntPtr hWnd, [MarshalAs(UnmanagedType.LPWStr)] string pszAppName, [MarshalAs(UnmanagedType.LPWStr)] string pszSubIdList);

        [DllImport("advapi32.dll", SetLastError = true)]
        internal static extern bool GetTokenInformation(IntPtr tokenHandle, NativeConstants.TokenInformationClass tokenInformationClass, IntPtr tokenInformation, int tokenInformationLength, out int returnLength);
